...
import { applyCors, sbAdmin, verifyAdminToken, isNonEmptyString, SLOT_KEY_RE, clubOrFilter, newId, sanitizeChapeauEntry, requireProAccess, resolveClubCaps, countComedians, PLAN_CAPS, COMEDIAN_PRIOS, stripMarkup } from './_lib.js';
...
function sanitizeComedian(row) {
  ...
  const out = {
    id: String(row.id),
    // Meme assainissement que cote portail : une seule regle pour toutes les
    // ecritures, quel que soit le chemin emprunte.
    name: stripMarkup(row.name, 200),
    ...
    phone: stripMarkup(row.phone, 40),
    email: typeof row.email === 'string' ? row.email.slice(0, 200) : '',
    notes: stripMarkup(row.notes, 2000),
