// ════════════════════════════════════════════════════════════════════════
// Tests — chantier multitenant, étape B (scoping serveur par club)
// ════════════════════════════════════════════════════════════════════════
...
//    requêtes DELETE/POST scopées produisent bien l'effet attendu une fois
//    exécutées côté serveur Postgres (ex. qu'une ligne club_id=NULL est bien
//    supprimée par `or=(club_id.eq.X,club_id.is.null)`). C'est un mécanisme
//    PostgREST standard et documenté, mais ce test ne l'exécute pas contre
//    une vraie instance. La vérification en conditions réelles (section
//    "Tests" du rapport livré avec ce chantier) reste à faire à la main après
//    déploiement, comme indiqué dans le plan (une ligne fictive d'un autre
//    club_id doit survivre à un clearAll du BCC).
...
test('clubOrFilter : le BCC (repli legacy) matche aussi club_id IS NULL', () => {
  const f = clubOrFilter(BCC_CLUB_ID);
  assert.match(f, /^or=\(club_id\.eq\.[^,]+,club_id\.is\.null\)$/);
  assert.match(f, new RegExp(encodeURIComponent(BCC_CLUB_ID)));
});

test('clubOrFilter : un vrai 2e club ne matche QUE son propre club_id (pas de repli NULL)', () => {
...
// ════════════════════════════════════════════════════════════════════════
// admin-login.js — repli mono-club (aucun appel réseau, pas de mock nécessaire)
// ════════════════════════════════════════════════════════════════════════

const adminLoginHandler = (await import('../api/admin-login.js')).default;

test('admin-login : mot de passe correct → token portant BCC_CLUB_ID (repli étape B)', async () => {
...
// ════════════════════════════════════════════════════════════════════════
// portal-write.js — résolution code → club, isolation
// ════════════════════════════════════════════════════════════════════════

const portalWriteHandler = (await import('../api/portal-write.js')).default;

test('code inconnu → 403, aucun appel Supabase déclenché', async () => {
  const mock = installFetchMock();
  ...
  assert.equal(mock.calls.length, 0, 'aucune écriture ne doit être tentée sans club résolu');
});

test('code BCCD25 valide → ensureComedian crée une ligne avec le club_id BCC', async () => {
...
test('recherche de comédien existant : le GET comedians inclut le repli club_id IS NULL pour le BCC (données pas encore backfillées)', async () => {
  const mock = installFetchMock();
  const req = fakeReq({ body: { action: 'ensureComedian', payload: {
  ...
  assert.ok(get.url.includes('is.null'), 'le repli legacy doit être présent tant que le backfill (étape C) n\'a pas eu lieu');
});
